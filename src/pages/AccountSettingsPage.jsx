...
import { LogOut, Trash2, Moon, Sun, CreditCard, Mail, Key, ChevronLeft, ExternalLink } from 'lucide-react';
...
export default function AccountSettingsPage() {
    ...
    base44.auth.me().then(u => {
      ...
      if (u?.email) {
        base44.entities.Purchase.filter({ buyer_email: u.email })
          .then(p => setPurchases(p.sort((a, b) => new Date(b.created_date) - new Date(a.created_date))))
          .catch(() => {});
      }
    }).catch(() => {});
  }, []);
  ...
  return (
    <div className="min-h-screen pb-32 dark:rave-bg" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
      {/* Header */}
      <div className="flex items-center gap-3 px-4 py-4 sticky top-0 z-10 frosted-bar" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 12px)' }}>
        <button
          onClick={() => navigate(-1)}
        ...
        <h1 className="font-display text-xl text-foreground">Account Settings</h1>
      </div>
      ...
      {user && <DeleteAccountModal user={user} isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} />}
    </div>
  );
}