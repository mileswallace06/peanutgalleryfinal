...
export default function SellingEventPicker({ initialKeyword = '', initialEventId, onSelect }) {
  ...
  return <section aria-label="Choose an event" className="space-y-5">
    ...
    {mode === 'live' && <p className="text-xs leading-relaxed text-muted-foreground rounded-xl border border-border p-3">Live Now covers known PG events, including labeled estimated windows. Ticketmaster currently supplies future events only, so provider live coverage is incomplete.</p>}
    ...
    {sourceError && <div role="alert" className="rounded-2xl border border-orange-400/30 bg-card p-4 space-y-2"><h2 className="font-bold">Some event results are unavailable</h2><p className="text-sm text-muted-foreground">{search.result.rateLimited ? 'Too many requests right now. Please wait a moment.' : 'We could not check every event source. This is not a complete set of local matches.'}</p><button type="button" onClick={search.refresh} className="min-h-11 font-bold text-primary">Retry search</button></div>}
    ...
    {!search.loading && visible.map(({ event, timing }) => <article key={event.tm_id || event.id} className="rounded-2xl border border-border bg-card overflow-hidden">
    ...
    {hasArea && <p className="text-xs leading-relaxed text-muted-foreground rounded-xl border border-border p-3">Live Now covers known PG events, including clearly marked estimated windows. Ticketmaster discovery currently supplies future events, so provider Live Now coverage is incomplete.{search.result.limited ? ' More PG events may exist beyond this result limit. Refine your search.' : ''}</p>}
  </section>;
}
