    ...
    <section aria-label="Feedback inbox" aria-busy={loading} className="px-4 py-5 space-y-4">
      ...
      <div className="space-y-3">
        {rows.map(row => {
          ...
          return <article key={row.id} aria-label="Feedback note" className="rounded-2xl border border-border bg-card p-4 space-y-3">
            ...
            {hideDetails ? <p className="text-sm text-muted-foreground italic">Note details hidden</p> : <>
              <p data-feedback-message className="text-sm text-foreground whitespace-pre-wrap [overflow-wrap:anywhere]">{row.message || 'No message included.'}</p>
              <p data-feedback-page className="text-xs text-muted-foreground [overflow-wrap:anywhere]">Page: {row.page || 'Unknown page'}</p>
            </>}
          </article>;
        })}
      </div>
