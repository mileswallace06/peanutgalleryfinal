...
| `base44/shared/buyerConfirmTransferCanaryOrchestrator.js` | P0-01T: Buyer-confirmation saga orchestrator — composes advisory buyer confirmation (`record_buyer_transfer_confirmation`) with financial capture (`runCanaryCaptureSaga`); authority-first with durable outbox; strips sensitive credentials from public response |
